...
test('Pi command handles paths with spaces and only displays a preview without a model turn', async () => {
  ...
  try {
    ...
    adapter({ registerCommand(name, definition) { assert.equal(name, 'forgeflow-plan-lanes'); command = definition; }, sendMessage(...args) { messages.push(args); } });
    const ctx = { cwd: dir, ui: { notify: (...args) => errors.push(args) } };
    await command.handler('"my brief.json"', ctx);
    assert.equal(errors.length, 0);
    assert.equal(messages.length, 1);
    adapter({ registerCommand(_name, definition) { command = definition; }, sendMessage() { throw 'message delivery failed'; } });
    await command.handler('my brief.json', ctx);
