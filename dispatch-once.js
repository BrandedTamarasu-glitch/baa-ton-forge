...
function dispatchable(report) {
  if (report.readiness?.state !== 'passed' || report.proposedStep?.code !== 'review-dispatch' || !report.proposedStep.workflowId)
...
export function registerDispatchOnce(pi, records, inspect = checkContinuation) {
  ...
  pi.registerCommand('forgeflow-dispatch-once', {
    description: 'Confirm and hand off one existing workflow to native Baa-ton dispatch; no retry',
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (!filename) throw new Error('Usage: /forgeflow-dispatch-once "path/to/brief.md"');
        ...
        const report = await inspectHere(source, ctx); dispatchable(report);
        if (records(ctx).some(item => item.kind === 'dispatch-intent' && item.workflowId === report.proposedStep.workflowId))
        ...
        const approved = await ctx.ui.confirm('Dispatch one existing workflow?',
          `Workflow: ${report.proposedStep.workflowId}\nTask: ${report.proposedStep.taskId}\nBrief: ${report.sourcePath}\nProfiles: ${JSON.stringify(report.readiness.evidence.profiles)}\nAllow one native herdr_dispatch attempt and a Pi model turn to request it? Baa-ton retains its own approval/startup checks. No restart, retry, verification, integration or next lane is authorized.`);
        if (!approved) { ctx.ui.notify('Dispatch handoff cancelled; nothing submitted', 'info'); return; }
        const fresh = await inspectHere(source, ctx); dispatchable(fresh);
  ...
  pi.on('tool_call', async (event, ctx) => {
    ...
    checking = true;
    try {
      const fresh = await inspectHere(intent.sourcePath, ctx); dispatchable(fresh);
