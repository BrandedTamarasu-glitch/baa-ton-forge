...
// All Herdr calls are read-only argv invocations through the native extension.
// Do not call bootstrap/doctor/config-dir helpers that may repair state here.
export async function nativePreflight({ prepared, sessionFile, exec, env = process.env, signal }) {
  ...
  const configDir = env.HERDR_PLUGIN_CONFIG_DIR || path.join(homedir(), '.config/herdr/plugins/config/herdr-orchestrator-controller');
  ...
  const sourceWorkspaceId = required(source, 'source_workspace_id', 'Application source workspace is unavailable');
